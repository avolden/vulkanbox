...
namespace MTL
{
	...
	using SharedEventNotificationBlock = void (^)(SharedEvent* pEvent, uint64_t value);
	using SharedEventNotificationFunction =
		MtlCpp::Function<void(SharedEvent* pEvent, uint64_t value)>;
	...
	public:
		...
		SharedEventListener* init(dispatch_queue_t const dispatchQueue);
	...
	public:
		...
		void notifyListener(MTL::SharedEventListener const* listener, uint64_t value,
		                    MTL::SharedEventNotificationBlock const block);
		void notifyListener(MTL::SharedEventListener const* listener, uint64_t value,
		                    MTL::SharedEventNotificationFunction const& function);
...
_MTL_INLINE void MTL::Event::setLabel(NS::String const* label)
{
...
_MTL_INLINE MTL::SharedEventListener*
			MTL::SharedEventListener::init(dispatch_queue_t const dispatchQueue)
{
...
{
	return Object::sendMessage<MTL::SharedEventListener*>(
		_MTL_PRIVATE_CLS(MTLSharedEventListener), _MTL_PRIVATE_SEL(sharedListener));
}
...
_MTL_INLINE void
MTL::SharedEvent::notifyListener(MTL::SharedEventListener const* listener, uint64_t value,
                                 MTL::SharedEventNotificationBlock const block)
{
	Object::sendMessage<void>(this, _MTL_PRIVATE_SEL(notifyListener_atValue_block_),
                                 ...
                                 MTL::SharedEventNotificationFunction const& function)
{
	...
	notifyListener(listener, value, ^void(SharedEvent* pEvent, uint64_t innerValue) {
                                                          ...
                                                          uint64_t milliseconds)
{
	return Object::sendMessage<bool>(
		this, _MTL_PRIVATE_SEL(waitUntilSignaledValue_timeoutMS_), value, milliseconds);
}
