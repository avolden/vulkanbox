...
namespace NS
{
	...
	public:
		 ...
		 * Create a new null pointer.
		 */
		SharedPtr(MtlCpp::nullptr_t) noexcept;

		/**
		 * SharedPtr copy constructor.
		 */
		SharedPtr(SharedPtr<_Class> const& other) noexcept;
		...
		SharedPtr(
			SharedPtr<_OtherClass> const& other,
			typename MtlCpp::enable_if_t<
				MtlCpp::is_convertible_v<_OtherClass*, _Class*>>* = nullptr) noexcept;
		 ...
		 * Copying increases reference count. Only releases previous pointee if objects
		 * are different.
		 */
		SharedPtr& operator=(SharedPtr<_Class> const& other);
		 ...
		 */
		template <class _OtherClass>
		typename MtlCpp::enable_if_t<MtlCpp::is_convertible_v<_OtherClass*, _Class*>,
		                             SharedPtr&>
		operator=(SharedPtr<_OtherClass> const& other);

		/**
		 * Move assignment operator.
		 * Move without affecting reference counts, unless pointees are equal. Moved-from
		 * object is reset to nullptr.
		 */
		...
		/**
		 * Access raw pointee.
		 * @warning Avoid wrapping the returned value again, as it may lead double frees
		 * unless this object becomes detached.
		 */
		_Class* get() const;
	 ...
	 * SharedPtr. Does not increase the reference count of the passed-in pointer, it is
	 * assumed to be >= 1. This method does not remove objects from an AutoreleasePool.
	 */
	template <class _Class>
...
template <class _Class>
_NS_INLINE NS::SharedPtr<_Class>::SharedPtr(MtlCpp::nullptr_t) noexcept
: m_pObject(nullptr)
{}

template <class _Class>
_NS_INLINE NS::SharedPtr<_Class>::SharedPtr(SharedPtr<_Class> const& other) noexcept
...
_NS_INLINE NS::SharedPtr<_Class>::SharedPtr(
	SharedPtr<_OtherClass> const& other,
	typename MtlCpp::enable_if_t<
		MtlCpp::is_convertible_v<_OtherClass*, _Class*>>*) noexcept
...
_NS_INLINE NS::SharedPtr<_Class>&
		   NS::SharedPtr<_Class>::operator=(SharedPtr<_Class> const& other)
	__attribute__((no_sanitize("undefined")))
{
...
typename MtlCpp::enable_if_t<MtlCpp::is_convertible_v<_OtherClass*, _Class*>,
                             NS::SharedPtr<_Class>&>
	_NS_INLINE NS::SharedPtr<_Class>::operator=(SharedPtr<_OtherClass> const& other)
		__attribute__((no_sanitize("undefined")))
{
	_Class* pOldObject = m_pObject;
...
_NS_INLINE bool operator==(NS::SharedPtr<_ClassLhs> const& lhs,
                           NS::SharedPtr<_ClassRhs> const& rhs)
{
